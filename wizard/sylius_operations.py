...
class SyliusOperations(models.TransientModel):
    _name = 'sylius.operation'
    ...
    def create_customer(self, customers):
        for customer in customers:
            ...
            cust_name = customer['email']
            c_name = cust_name.split("@")[0]
            values = {
                'name': c_name,
                'customer': True,
            ...
            partner_id = request.env['res.partner'].sudo().search([('email', '=', customer['email']),('customer','=','True')])
    ...
    def import_order_sylius(self, **post):
        ...
        while i <= 1:
            ...
            items = val['_embedded']['items']
            for item in items:
                detailed_page_url = 'https://staging.brandsofpuertorico.com/api/v1/orders/' + str(item['id'])
    ...
    def create_order(self, orders):
        for order in orders:
            ...
            if sale_order_id:
                continue
            if order.get('state') != 'cart':
                if 'customer' in order:
                    self.create_customer([order['customer']])
                customer_id = request.env['res.partner'].sudo().search([('name', '=', order['shippingAddress']['firstName'] +" " + order['shippingAddress']['lastName']),('customer','=','True')],limit=1)
                if 'phoneNumber' in (order['shippingAddress']):
                    ship_phone = order['shippingAddress']['phoneNumber']
                if order['shippingAddress']['countryCode']:
                    c_code = self.env['res.country'].search([('code', '=', order['shippingAddress']['countryCode'])], limit=1)
                if 'apartment' in (order['shippingAddress']):
                ...
                if not customer_id:
                    customer_id = request.env['res.partner'].sudo().create({
                                                              ...
                                                              'customer' :True,
                                                              'street': order['shippingAddress']['street'] +" " + apartment,
                                                              'city':order['shippingAddress']['city'],
                                                              'zip':order['shippingAddress']['postcode'],
                                                              'phone':ship_phone,
                                                              'email':order['customer']['email'],
                                                              'country_id': c_code.id})
                del_add = customer_id.address_get(['delivery'])
                    # c_code = order['shippingAddress']['countryCode']
                # try:
                #     if 'phoneNumber' in (order['shippingAddress']):
                #         values = {
                ...
                #             'name': order['shippingAddress']['firstName'] +" " + order['shippingAddress']['lastName'],
                #         }
                #         if customer_id.phone is False:
                #             customer_id.write(values)
                # except KeyError:
                #     print("Phone Number is not present")
                if not customer_id.type == 'delivery':
                ...
                bill_add = customer_id.address_get(['invoice'])
                if 'phoneNumber' in (order['billingAddress']):
                    bill_phone = order['billingAddress']['phoneNumber']
                if not customer_id.type == 'invoice':
                    bill_add =  request.env['res.partner'].sudo().create({'parent_id':customer_id.id,
                                                                  'type':'invoice',
                ...
                if not sale_order_id:
                    ...
                    if sale_order_id:
                        sale_order_id.write({'name':"BPR/"+sale_order_id.name})
                    print(sale_order_id)
                ...
                for line in order['items']:
                    ...
                    if not prod_id:
                        product_url_detailed = 'https://staging.brandsofpuertorico.com' + \
                                               line['variant']['_links']['product']['href']
                        product_detailed_res = requests.get(product_url_detailed,
                                                            headers={'Authorization': self.get_auth_token_header(),
                                                                     'Accept': 'application/json', })
                        val_detailed = json.loads(product_detailed_res.text)
                        self.create_product([val_detailed])
                        prod_created_id = request.env['product.template'].search(
                            [('sylius_id', '=', val_detailed['id'])])
                        var_vise_url = 'https://staging.brandsofpuertorico.com/index.php/api/v1/products/' + prod_created_id.sylius_code + '/variants'
                        r = requests.get(var_vise_url, headers={'Authorization': self.get_auth_token_header(),
                                                                'Accept': 'application/json', })
                        val = json.loads(r.text)
                        prod_var = []
                        items = val['_embedded']['items']
                        for item in items:
                            variant_vise_url = 'https://staging.brandsofpuertorico.com/index.php/api/v1/products/' + prod_created_id.sylius_code + '/variants/' + \
                                               item['code']
                            r_var = requests.get(variant_vise_url,
                                                 headers={'Authorization': self.get_auth_token_header(),
                                                          'Accept': 'application/json', })
                        ...
                        if val['total'] > 1:
                            ...
                            for att_val in prod_var:
                                ...
                                if product_var_id:
                                    ...
                                    if 'weight' in att_val:
                                    # if att_val.has_key('weight'):
                                        product_id_variant.write({'weight': att_val['weight']})

                            prod_id = request.env['product.product'].search(
                                [('sylius_variant_id', '=', line['variant']['id'])])

                        else:
                            for prod_var_new in prod_var:
                                prod_created_id.valid_existing_variant_ids.write(
                                    {'sylius_variant_id': prod_var_new['id'],
                                     'sylius_variant_code': prod_var_new['code']})
                                if 'weight' in prod_var_new:
                                # if prod_var_new.has_key('weight'):
                                    prod_created_id.valid_existing_variant_ids.write({'weight': prod_var_new['weight']})
                                prod_id = prod_created_id.valid_existing_variant_ids

                    # prod_variant_id = request.env['product.template'].search([('id', '=', prod_id.id),
                    #                                                           ('attribute_id', '=', prod_id.id)])
                    if 'name' in line['variant']['translations']['en']:
                        desc = line['variant']['translations']['en']['name']
                    else:
                        desc = prod_id.name
                    vals_order_line = {'order_id': sale_order_id.id,
                                       ...
                                       # 'name': prod_id.name,
                                       # 'name': desc,
                                       'name': prod_id.partner_ref,
                                       'product_uom_qty': line['quantity'],
                                       'price_unit': line['unitPrice'] / 100}

                    orders_line = request.env['sale.order.line'].create(vals_order_line)
                    sales_order_line.append(orders_line.id)
                sale_order_id.write({'order_id' : [(6, 0, sales_order_line)]})
                ...
                if order['adjustments']:
                    for adjustmentlines in order['adjustments']:
                        ...
                        adj_line = request.env['sale.adjustment.lines'].create(values_adjustment_lines)
                        adj_list.append(adj_line.id)
                    sale_order_id.write({'sale_adjustment_lines_ids' : [(6, 0, adj_list)]})
                    ...
                    if product.product_variant_ids:
                        for product_var in product.product_variant_ids:
                            ...
                            for var in product_var.attribute_value_ids:
                                attr_id = request.env['product.attribute'].search(
                                    [('id', '=', var.attribute_id.id)])
                                if attr_id:
        ...
        if produtcs:
            ...
            for product in produtcs:
                ...
                if product.attribute_line_ids:
                    ...
                    for attribute in product.attribute_line_ids:
                        ...
                        options_response = requests.post(options_url, data=json.dumps(options_data),
                                                         headers=headers)
                        if options_response.status_code == 400:
                            options_update_url = "https://staging.brandsofpuertorico.com/index.php/api/v1/product-options/" + (
                                attribute.attribute_id.name).lower()
                            options_update_response = requests.patch(options_update_url,
                                                                     data=json.dumps(options_data),
                                                                     headers=headers)
                ...
                if response.status_code == 201:
                    ...
                    report = request.env['sylius.report'].create(
                        {'product_id': product.id, 'export_date': datetime.datetime.now(), 'status': 'Fail',
                         'status_code': response.status_code,
                         'reason': json.loads(response.content.decode('utf-8'))})
                    res = json.loads(response.content.decode('utf-8'))
    ...
    def add_product_variants(self, **post):
        ...
        if prods:
            for prod in prods:
                ...
                products_var = []
                while i <= pages:
                    page_vise_url = 'https://staging.brandsofpuertorico.com/index.php/api/v1/products/' + prod.sylius_code + '/variants/?page=' + str(
                        i)
                    time.sleep(0.01)
                    r = requests.get(page_vise_url, headers=headers_product_variant)
                    val = json.loads(r.text)
                    items = val['_embedded']['items']
                    for item in items:
                        variant_vise_url = 'https://staging.brandsofpuertorico.com/index.php/api/v1/products/' + prod.sylius_code + '/variants/' + \
                                           item['code']
                    ...
                    if val['total'] > 1:
                        ...
                        for product in products_var:
                            ...
                            vals_ids.append(value_id.id)
                        attribute_template_line_id = request.env['product.template.attribute.line'].sudo().create(
                        ...
                        for prod_var_prod in products_var:
                            ...
                            if 'weight' in prod_var_prod:
                                prod.valid_existing_variant_ids.write({'weight': prod_var_prod['weight']})
